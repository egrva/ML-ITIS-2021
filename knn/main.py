...
class Scene:
    ...
    def draw_description(self, clusters):
        ...
        if self.still_training:
            surf = font.render(f'Num keys to choose drawing color. [Q] to start KNN, [Esc] to exit.',
                               False, colors[-1])
            screen.blit(surf, (3, 0))
        else:
            surf = font.render(f'KNN started. RMB to calculate optimal k, LMB to use calculated k.  '
